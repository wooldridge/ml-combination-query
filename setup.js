...
function loadData() {
  ...
  var options = {
    method: 'PUT',
    uri: 'http://' + config.host + ':' + config.restSetup["rest-api"]["port"] + '/v1/documents?database=' + config.databaseSetup["database-name"] + '&uri=/documents/' + currFile + '&collection=docs',
    body: buffer,
    auth: config.auth
  };
  rp(options)
...
var dataPath2 = config.path + 'docs2/'
    dataFiles2 = fs.readdirSync(dataPath2),
    count2 = 0;

function loadData2() {
  var currFile = dataFiles2.shift();
  count2++;
  var buffer;
  buffer = fs.readFileSync(dataPath2 + currFile);

  var options = {
    method: 'PUT',
    uri: 'http://' + config.host + ':' + config.restSetup["rest-api"]["port"] + '/v1/documents?database=' + config.databaseSetup["database-name"] + '&uri=/locations/' + currFile + '&collection=places',
    body: buffer,
    auth: config.auth
  };
  rp(options)
